    ...
    public class MainViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        private void LoadDummyData()
        {
            Patients = new ObservableCollection<PatientViewModel>
            {
                new PatientViewModel
                {
                    ...
                    DossierMedical = new DossierMedicalViewModel
                    {
                        GroupeSanguin = "A+",
            ...
            // Ajouter des consultations au patient
            var patient = Patients[0];
            patient.Consultations = new ObservableCollection<ConsultationViewModel>
            {
                ...
                new ConsultationViewModel
                {
                    ...
                    Observations = "Douleur localisée région lombaire",
                    Diagnostic = "Lombalgie"
                }
            };

            // Ajouter des prescriptions au patient
            patient.Prescriptions = new ObservableCollection<PrescriptionViewModel>
            {
                new PrescriptionViewModel
                {
                    DatePrescription = DateTime.Now.AddDays(-30),
                    Medicament = "Paracétamol",
                    Posologie = "1000mg 3x/jour",
                    Duree = "7 jours",
                    Etat = "Terminée",
                    DateCloture = DateTime.Now.AddDays(-23)
                },
                new PrescriptionViewModel
                {
                    DatePrescription = DateTime.Now.AddDays(-15),
                    Medicament = "Ibuprofène",
                    Posologie = "400mg 2x/jour",
                    Duree = "5 jours",
                    Etat = "En cours"
                }
            };
        ...
        private void ExecuteSupprimerPatient()
        {
            ...
            if (result == MessageBoxResult.Yes)
            {
                try
                {
                    // Supprimer d'abord les prescriptions
                    if (SelectedPatient.Prescriptions != null)
                        SelectedPatient.Prescriptions.Clear();

                    // Supprimer les consultations
                    if (SelectedPatient.Consultations != null)
        ...
        private void ExecuteAjouterPatient()
        {
            ...
            // Créer un nouveau patient avec l'ID généré
            var nouveauPatient = new PatientViewModel
            {
                Identifiant = newId,
                DateNaissance = DateTime.Now, // Valeur par défaut qui pourra être modifiée
                Consultations = new ObservableCollection<ConsultationViewModel>(),
                Prescriptions = new ObservableCollection<PrescriptionViewModel>()
            };
            
            // Définir le nouveau patient comme patient sélectionné
            SelectedPatient = nouveauPatient;
            
            IsNewPatient = true;
            SelectedTabIndex = 0; // Aller à l'onglet Fiche Patient
        ...
        private void ExecuteModifierPatient()
        {
            ...
            // Aller à l'onglet de la fiche patient
            SelectedTabIndex = 0;
            
            // Optionnel : Afficher un message de confirmation
            MessageBox.Show("Vous pouvez maintenant modifier les informations du patient.", 
                          "Mode édition", 
                          MessageBoxButton.OK, 
